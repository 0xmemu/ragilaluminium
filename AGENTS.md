...
- Batch polish #3 (commit berikutnya): label "Pilih varian" kembali 14px bold di tempat semula + baris varian&stok (font light) sebaris rata kanan dengan label; heading benefit jadi "Alasan harus belanja di Ragil Aluminium"; rating pindah kanan sejajar baris 1 judul dengan frame rounded; nama produk di-split otomatis (baris 1 ukuran "Tinggi …cm × Panjang …cm", baris 2 model) via regex — tidak butuh setting admin. Terverifikasi live.
