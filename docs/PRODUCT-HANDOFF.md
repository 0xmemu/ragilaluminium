...
- **CTA Storefront (`admin.cta-settings.*`)** - mengatur teks dua jenis blok lewat prop
  `ctaSettings`: (a) banner penutup per halaman publik (kunci: home, model-detail, about, faq,
  cara-pemesanan, masalah-solusi), dan (b) kartu reusable yang dipakai berulang di halaman
  transaksi: `trust` (kartu jaminan "Belanja Aman & Terpercaya", dipakai di keranjang, checkout,
  konfirmasi pesanan, daftar pesanan, pelacakan) dan `order-help` (blok bantuan di halaman
  pelacakan pesanan). Hanya kop dan judul yang dapat diubah; tombol/aksi tetap di kode.
