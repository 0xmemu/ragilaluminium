...
- `GET /products/{parent_sku}` -> `ProductController@show`  (name: `product.slug`)
- `GET /categories/{slug}` -> `StorefrontCategoryController@show`  (name: `storefront.category`)
- `GET /collections/{slug}` -> `CatalogController@collectionShow`  (name: `storefront.collection`)
...
- `GET /search` -> `CatalogController@search`  (name: `search`)
