...
| Import dan media | file bomb, MIME spoofing, SSRF, redirect ke jaringan privat, resource exhaustion, formula injection | ukuran dan extension import dibatasi, media MIME/magic bytes dan byte limit, host allowlist + blok IP privat, DNS kosong ditolak fail-closed, formula-cell sanitizer + batas export 50000 baris, outbound redirect dimatikan, queue retry/unique/timeout | retention policy PII ditetapkan via ADR-017; PII export review masih perlu evidence eksplisit |
