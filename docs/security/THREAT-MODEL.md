...
**Cakupan:** storefront, checkout/order, admin, import/media, webhook J&T, WhatsApp Meta/WAHA, queue, storage, dan CI.
...
4. Laravel keluar menuju J&T, Meta/WAHA, media CDN, object storage, database, cache, dan queue.
...
| Meta/WAHA | forged webhook, query secret leak, duplicate message/action, provider outage | signature/secret wajib, query secret ditolak, production boot gate, rate limit, safe logging, state transition idempoten | event/message replay ledger menyeluruh dan delivery SLI belum tersedia |
