...
import type { MediaPanelUrls } from "@/components/admin/product-edit/types"
...
export function MediaRowsPanel({
  product,
  variants = [],
  library = [],
  urls,
  ...
  product: { id: number; parent_sku: string }
  variants: VariantOption[]
  ...
  // Filter varian & pencarian library dilakukan di sisi klien terhadap rows
  // yang dikirim server (tanpa round-trip Inertia; rows statis per muat halaman).
  const filteredRows = React.useMemo(() => {
    if (variantFilter === "shared") return rows.filter((r) => r.product_variant_id === null)
    if (variantFilter) return rows.filter((r) => r.product_variant_id === Number(variantFilter))
    return rows
  }, [rows, variantFilter])
  ...
  async function uploadDirect(file: File) {
    ...
    try {
      const presignRes = await fetch(urls.presignUrl, {
        method: "POST",
        headers: {
  ...
  function toggleSelectAll() {
    const pageIds = filteredRows.map((r) => r.id)
  ...
  React.useEffect(() => {
    ...
    async function tick() {
      ...
      try {
        ...
        const res = await fetch(`${urls.statusUrl}?${params.toString()}`, {
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          {filteredRows.length} dari {rows.length} media
        </p>
      </div>
                ...
                {LIBRARY_CONTEXT_PRESETS.map((preset) => {
                  ...
                  return (
                    <button
                      key={preset.label}
                      type="button"
                      onClick={() => setLibrarySearch(preset.q)}
                      className={cn(
