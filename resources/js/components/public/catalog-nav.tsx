...
export function CatalogNav({
  ...
  return (
    <section className="border-b border-border bg-surface" aria-label="Navigasi katalog produk">
      {/* Baris 1 — Tab kategori & model, underline tebal pada tab aktif */}
      <nav aria-label="Kategori produk" className="scrollbar-x -mb-px flex gap-1 overflow-x-auto">
          ...
          {CATEGORY_TABS.map((tab) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "relative flex h-12 shrink-0 items-center whitespace-nowrap border-b-2 px-0.5 text-sm transition",
                  active
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                className="flex h-12 shrink-0 cursor-pointer items-center gap-0.5 border-b-2 border-transparent px-1 text-sm text-muted-foreground transition hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
              >
                Kategori
      ...
      {/* Baris 2 — Filter bar: Filter, flashsale, Kategori, Model, Desain */}
      <div className="border-b border-border bg-surface-muted">
        <div className="scrollbar-x flex items-center gap-2 overflow-x-auto px-5 py-2.5 md:px-8 lg:px-12">
      ...
      {/* Baris 3 — Info hasil: nama kategori, jumlah barang, Atur */}
      <div className="container-page flex items-center justify-between gap-3 px-5 py-3 md:px-8 lg:px-12">
        <h1 className="min-w-0 truncate text-sm font-bold tracking-tight text-foreground">
          {categoryName}
        </h1>
