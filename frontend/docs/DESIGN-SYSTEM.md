...
- Section public: ritme vertikal via utility `.section-space` — `padding-block: 1.75rem` (28px) mobile, `2.75rem` (44px) mulai `56.25em`. Home memakai band `bg-surface` / `bg-surface-muted` selang-seling + heading section `size="display"`. Strip carousel berurutan tidak memakai padding 80px agar jarak antar section tidak terasa jauh.
