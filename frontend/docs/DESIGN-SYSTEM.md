...
- Sudut (radius): panel, filter sheet, dan surface utilitas memakai token `--radius` (14px). Search bar, input satu baris, dan **tombol aksi berlabel** memakai `rounded-full` (pill). **Hanya listing cards** (product / model / testimonial / galeri hasil pemasangan) yang wajib sudut siku — frame dan media 1:1 tanpa `rounded-*`. Chip/badge singkat dan kontrol ikon lingkaran tetap `rounded-full`.
...
- Galeri Model Produk dan section Paling banyak dipesan di Home memakai horizontal scroll-snap dengan tombol back/next yang sama seperti banner promosi: lingkaran putih semi-opak, ikon Graphite, shadow, dan hover scale. Tombol duduk di tepi kiri/kanan track (setengah di dalam, setengah di luar) agar tidak menutupi card saat padding section tipis. Paling banyak dipesan memuat maksimal 10 produk nyata yang dipilih lewat flag admin `homepage_popular`, dengan tepat 4 kartu per tampilan desktop (2 mobile, 3 small tablet). Tombol hanya tampil bila masih ada konten pada arah tersebut, memiliki label aksesibel, dan menghormati reduced motion. Catalog / Search / PDP related tetap memakai `ProductCardGrid`.
...
- Search bar desktop (dan dialog mobile) memakai placeholder berputar (`Cari produk...`, model, ukuran, dll.) via `useRotatingPlaceholder`; berhenti saat fokus/isi teks atau `prefers-reduced-motion`.
- Header memakai hamburger di kiri logo pada semua breakpoint. Desktop menampilkan kembali strip navigasi utama di bawah bar logo/search dari `config/sitemap.php → navigation.desktop_main` (**Flash Sale** pertama dengan ikon petir, Signal Red, hover putih; tanpa item Promo); strip kategori lama tetap tidak dipakai.
- **Announcement ticker** (bar Signal Red di atas header): copy sentence-case fokus model/benefit (bukan ALL CAPS). Flash Sale di ticker **maksimal 1 item** dan hanya saat periode kampanye live (`FlashSalePeriodSettings`); teks "Flash Sale" dari config/CMS/banner difilter. Ikon konteks + chip persen putih bila ada `-N%`. Hover menjeda marquee.
- Drawer membuka dari kiri dengan surface polos tanpa border, divider, atau shadow. `Model Produk` dan `Semua Produk` menjadi heading utama. Hover/focus pada `Model Produk` membuka panel taxonomy model visible; tap caret menjadi fallback layar sentuh.
