...
Operational layout and consistency rules live in
`frontend/docs/UI-CONSISTENCY-CONTRACT.md`. This file remains the token and
principle reference; the consistency contract is the cross-page implementation
gate.
...
- `foreground`: `#333333` untuk heading dan teks utama
- `action`: `#262626` untuk teks aksi netral
- `body` / `muted-foreground`: `#737373` untuk deskripsi, artikel, dan body text panjang
...
- Public navigation chrome (header, navbar, footer, dan mobile bottom navigation) memakai `#262626` melalui token `bg-action`; teks dan ikon memakai `text-background`.
- Section public: ritme vertikal via utility `.section-space` — `padding-block: 1.75rem` (28px) mobile, `2.75rem` (44px) mulai `56.25em`. Home memakai band `bg-surface` / `bg-surface-muted` selang-seling dengan seluruh heading section H2 tetap `18px`. Strip carousel berurutan tidak memakai padding 80px agar jarak antar section tidak terasa jauh.
- Admin: 20-32px panel spacing, 12-20px internal spacing.
- Hero memakai `min-height` berbasis `dvh`, bukan `h-screen`.
- Hero Home adalah campaign slider full-bleed (lebar penuh layar, gaya campaign IKEA): track `translateX` per slide. **Standar gambar banner PATEN (2026-08-14): rasio layout tetap `1024/426` (±2.4:1) di semua breakpoint (`aspect-[1024/426]`) — tinggi otomatis = lebar ÷ rasio sehingga proporsional di desktop maupun mobile; gambar dirender `object-cover` (admin menyiapkan canvas 2,4:1, mis. `2048 × 852 px`, agar tidak ter-crop).** Tombol back/next bulat putih di tepi kiri/kanan banner, dot indikator di bawah, auto-advance 5 detik, dan menghormati reduced motion. **Upload banner admin memakai alur presigned langsung ke R2 + derivatif WebP (thumb/card/pdp) via queue** — slide landing memakai `pdp` (maks 1400px, `image/webp`), bukan PNG asli upload (bisa 1 MB+), sehingga hero lebih cepat termuat. CTA graphite `Belanja Sekarang` (`rounded-full`).
- Tipografi global memakai Apple system font (`-apple-system`, `BlinkMacSystemFont`, `Segoe UI`, sans-serif). Capitalize pada heading/nav/CTA saja; body/subtitle sentence case. **Jangan em dash (—)** pada copy pelanggan; pakai titik, koma, atau titik dua. Hindari kata informal seperti “mencolok” / “rekaan”.
...
- `layout: promo_card` / `sticker: true` (standar banner manual + otomatis + fallback promo): foto produk full-bleed di belakang, banner dan kartu rasio 3:4 di kiri memakai `rounded-md` (radius `8px`), tinggi kartu 72% banner, padding dalam mobile `px-5 py-5` / `sm+` `px-8 py-9`, drop shadow lembut. Warna kartu dirotasi per posisi slide dari 3 variasi brand: (1) Signal Red `bg-primary` teks putih + chip persen putih berteks merah, (2) Graphite `#1A1D1C` teks putih + chip persen merah, (3) Aluminium `#DDE2E0` teks Graphite + chip persen merah + CTA Graphite. Struktur konten sama di semua variasi, dimulai dari headline **model produk** (mis. `Boven\nJungkit` / `Jendela\nSwing`, bukan ukuran SKU seperti `110x70`), chip persen opsional, subheadline `Harga miring, kualitas terjamin`, CTA "Belanja sekarang" (`rounded-full`) menuju listing model, disclaimer kecil dari `slide.disclaimer`.
...
- Model card memakai judul `h3` 14px dengan meta/desc 12px. Area teks ~5.75–7rem.
- Section **Kami bantu dari awal sampai jadi** memakai empat kartu infografis, satu kartu per poin, dengan skala lebih kecil dari banner promo, `rounded-md`, ikon, nomor urut, judul, dan deskripsi. Kartu tetap memakai data nyata dari `HELP_STEPS` dan CTA katalog/WhatsApp di bawahnya.
...
- Galeri Model Produk, Paling banyak dipesan, Hasil pemasangan, dan Ulasan di Home memakai horizontal scroll-snap (maks. 10 item). Strip **Paling banyak dipesan**: judul section di permukaan halaman biasa; **Signal Red (`bg-primary`) full-bleed ke samping**, tinggi band **mengikuti tinggi kartu** (gutter tipis `py-2`, tanpa padding berlebih). Dipakai juga di hub **Semua Model Produk** (`/products`) dan listing **Semua Produk** (`/products?sort=…`) via `PalingBanyakDipesanSection` + props `popularProducts` (kurasi `homepage_popular`, fallback penjualan). **Desktop (`md+`):** tombol back/next lingkaran di **dalam** tepi track (`left-2`/`right-2`, bukan half-outside) agar tidak terpotong overflow; hanya tampil bila masih ada konten pada arah tersebut, label aksesibel, hormati reduced motion. **Mobile:** tanpa tombol next/back — navigasi sepenuhnya swipe (`touch-pan-x` + snap); setelah kartu terakhir ada slot **Lihat semua** yang mengarah ke route listing nyata (`/products`, `?sort=popular`, `/hasil-pemasangan`, `/reviews`). CTA section memakai label **Lihat semua →** berukuran `12px` dan `font-bold`, lalu disembunyikan di mobile agar fokus swipe. Banner promo mobile juga swipe (tanpa panah), dots tetap. Catalog / Search / PDP related tetap memakai `ProductCardGrid`.
...
- Header publik = action graphite gelap `#262626` (`bg-action`), bukan strip merah penuh; Signal Red untuk aksen (search, badge, active indicator, Flash Sale).
...
- Search bar header memakai tinggi `36px` (`h-9`) dan placeholder berputar (contoh: `Cari: jendela sliding / 120x80`) via `useRotatingPlaceholder`; berhenti saat fokus/isi teks atau `prefers-reduced-motion`.
...
- Mobile bottom navigation (`MobileBottomNav`): tinggi tetap `h-14` + `safe-area-inset-bottom`, `max-w-[100vw]` + grid kolom sama lebar (`minmax(0,1fr)`), ikon Phosphor statis `22px`, label `10px` truncate, indikator aktif `w-7`, `z-[60]`.
- Storefront layout + home carousels: `overflow-x-clip` / `min-w-0` agar track horizontal tidak melebarkan dokumen (yang sebelumnya mendorong tab kanan nav keluar viewport).
- Home rails: touch native `touch-action: pan-x pan-y` + `snap-proximity` (swipe kartu + scroll halaman); `useDragScroll` hanya mouse-drag + inertia desktop, dan menahan klik kartu setelah swipe.
...
- Drawer hamburger: tombol caret di **Model Produk** membuka panel daftar model di **kanan** (nav menyusut ke kiri) — pola yang sama di mobile dan desktop, bukan accordion di bawah.
...
mengembalikan light agar toko publik tidak ikut gelap. Netral mode gelap mengikuti dasar
sidebar `#131212` (bukan tint mineral hijau).
